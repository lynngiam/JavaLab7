...
public class RBTreeMap<K extends Comparable<? super K>, V> implements MapADT<K, V> {
    ...
    RBTreeMap<K, V> left, right;
    /** subtrees of this subtree -- if both null, this tree is empty */
    ...
    protected V put(K key, V value, RBTreeMap<K, V> parent, RBTreeMap<K, V> gran) { // Lynn
	...
	if (isEmpty()) { // base case I: empty tree
		...
		left.put(key, value, this, parent);

	    } else { // recursive case II: put in right subtree
		right.put(key, value, this, parent);
	    }
	}
	...
	if (parent == null) {
		...
		// TODO: Implement this case
		/*
		 * You need to determine which rotation case (LL, LR, RL, RR) to apply. For
		 * example, if this.key < parent.key < gran.key, this is the LL case; if
		 * this.key < parent.key and gran.key < parent.key, this is the RL case
		 */
		if (this.key.compareTo(parent.key) < 0) {
		    if (parent.key.compareTo(gran.key) < 0) {
			rotateRight(gran); // LL; single right rotation
		    } else if (parent.key.compareTo(gran.key) > 0) {
			...
			rotateLeft(gran); // RR; single left rotation
		    }
		}

		/*
		 * Then apply the appropriate rotation: LL: single right rotation LR: double
		 * right rotation (a left rotation at p followed by a right rotation at g) RR:
		 * single left rotation RL: double left rotation (a right rotation at p followed
    ...
    /**
     * Build a tree from existing components. To be used only within RBTreeMap and
     * subchildren.
     * 
     * @param key
    ...
    /**
     * Creates a StringBuffer (String takes too long) and then starts the recursive
     * traversal of nodes.
     * 
    ...
    /**
     * Determines the number of black nodes on any path from root to leaf in this
     * red-black tree
     * 
     * @return the
     */
    ...
    /**
     * Implementation of the clear method of java's Map interface Removes all
     * key-value pairs from this Map
     */
    public void clear() {
	color = BLACK;
	key = null;
